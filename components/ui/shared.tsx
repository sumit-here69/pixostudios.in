...
export function SectionHeader({
  ...
  return (
    <div className={`${center ? "text-center" : ""} mb-12 ${className}`}>
      ...
      <h2 className="text-[38px] sm:text-[48px] lg:text-[56px] font-bold leading-[1.08] tracking-[-0.02em] text-primary">
        {heading}
      </h2>
      {subtitle && (
    ...
    <div className={`flex items-center justify-center gap-3 ${className}`}>
      <button onClick={onPrev} className={NAV_ARROW_CLASSES} aria-label="Previous">
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
          <path d="M10 4L6 8L10 12" stroke="#060612" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </button>
      <button onClick={onNext} className={NAV_ARROW_CLASSES} aria-label="Next">
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
          <path d="M6 4L10 8L6 12" stroke="#060612" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </button>
    </div>
  );
...
export function SerifAccent({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <span
      className={`text-accent italic ${className}`}
